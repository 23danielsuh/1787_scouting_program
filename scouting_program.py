...
def process_dataframe(init_df):
    ...
    df["total_points"] = (
        df["total_auto_points"] + df["total_teleop_points"] + df["climb"] + df["taxi"]
    )
...
def write_data(team, data):
    ...
    average_auto = round(
        data["total_auto_points"].mean(axis=0) + data["taxi"].mean(axis=0), 2
    )
    ...
    averages = pd.DataFrame(
        [["N/A", average_auto, average_tele, average_climb, average_total_points]],
...
def write_statistics(team, data):
    ...
    if len(pd.unique(data["time"])) != 1:
        p_value = round(
            stats.ttest_ind(
                data["total_points"][data["time"] == 1],
                data["total_points"][data["time"] == 2],
            ).pvalue,
            7,
        )
    ...
    # writes data
    df.to_excel(
        writer, sheet_name=str(team), index=False, startrow=len(data) + 4, startcol=1
    )
...
def main():
    ...
    parser.add_argument(
        "--path",
        type=str,
        help="path to csv (probably somewhere in ~/Downloads). Defaults to using Sheets API.",
        default="",
    )
    ...
    for i, column in enumerate(columns):  # df -> df subset -> df????
        ...
        temp_df = temp_df.sort_values(
            by=[column], ascending=(column == "P-value"), ignore_index=True
        )
